...
print("Rave Event | griffindoescooking")
...
local main = quake:Window({
    Title = "Pet Simulator 99 | Rave Event",
...
checkActive("LuckyTilesInstance")
...
local farmEggs = eggTab:Toggle({
    ...
    Callback = function(value)
        ...
        HumanoidRootPart.CFrame = CFrame.lookAt(HumanoidRootPart.Position, egg.Position)
        task.wait(0.2)
...
task.spawn(function()
    farmEggs:Lock("Complete Egg selection")
    repeat
        task.wait()
    until hasChosenEgg
    farmEggs:Unlock()
